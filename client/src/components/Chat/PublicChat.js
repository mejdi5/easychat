...
<div className="chat">
            <div className="chat_header">
                <div className="chat_headerinfo">
                    ...
                    <div key={index}
                    style={{textAlign:'center', display:'flex', justifyContent:'center', marginBottom:'20px'}}
                    >
                    <img 
                    src={users && users.find(u => u._id === m).picture.path} 
                    alt=''
                    className="conversationImg"/>
                    <p>{users && users.find(u => u._id === m).firstName} {users && users.find(u => u._id === m).lastName}</p>
                    </div>
                    )}
                    </MDBModalBody>
                    </MDBModalContent>
            ...
            {messages && messages.map((message,index) => {
                const Sender = users && users.find(el => el._id === message.sender)
                const open = () => {
