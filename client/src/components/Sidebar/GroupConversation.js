...
function GroupConversation({currentChat, setCurrentChat, readingPublicConversation, publicNotifications, setPublicNotifications}) {
...
return (
    <div className="sidebar-chats" >
        ...
        {groupConversations.filter(gc => gc.members.some(m => m == user?._id)).length === 0 &&
        <div className="sidebarchat">No groups</div>
        }
        ...
        <div 
        className={
            currentChat === groupConversation 
            ? "sidebarchatactive" 
            : (publicNotifications.some(n => n.chatId === groupConversation._id) && (!currentChat || currentChat !== groupConversation))
            ? "readsidebarchat"
            : "sidebarchat"
        }
        >
            ...
            &&
            <>
            <div onClick={() => readingPublicConversation(groupConversation)} style={{display:'flex'}}>
            <img
